...
class Controller(imageProcessing):
    ...
    def computeError(self):
        ...
        # self.center = int(Max - ((Max - Min) * 1 / 4))
        print("Min: {}, Max: {}, Center: {}".format(Min, Max, self.center))
        self.error = int(self.mask.shape[1] * 1 / 2) - self.center
        return self.error
    ...
    def PIDController(self, error):
        ...
        # D = (error - self.error_arr[1]) / 1 * self.d
        # I = np.sum(self.error_arr) * 1 * self.i
        angle = P + I + D
        if abs(angle) > 5:
